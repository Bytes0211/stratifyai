...
def chat(
    ...
    """
    return _chat_impl(
        message, provider, model, temperature, max_tokens, stream, system, file,
        cache_control, chunked, chunk_size, auto_select=auto_select,
...
def templates(
    tag: Optional[str] = typer.Option(
        None,
        "--tag",
        help="Filter templates by tag (e.g., code, writing, data)"
    ),
    ...
    try:
        ...
        # Create table
        title = f"📋 Prompt Templates" + (f" [dim](tag: {tag})[/dim]" if tag else "")
        ...
        console.print("\n  Use a template:")
        console.print("    [green]stratifyai chat --template code_review --params 'language=python,focus=security' --file script.py[/green]")
        console.print()

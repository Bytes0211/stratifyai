...
# Install dev/test dependencies (includes runtime)
...
# Primary Providers
STRATIFYAI_API_KEY=replace-with-a-strong-random-token
...
Use reusable, parameterized prompt templates for common tasks.

### Built-in Templates

StratifyAI includes 10 production-ready templates:

- **code_review** - Review code for bugs, security, performance
- **summarize** - Summarize documents with configurable length/style
- **chatbot** - Conversational assistant with custom persona
- **explain_concept** - Explain complex topics at different levels
- **analyze_data** - Analyze CSV, JSON, or tabular data
- **rag_synthesis** - Synthesize answers from retrieved context
- **translate** - Translate text between languages
- **debug_error** - Debug error messages and stack traces
- **commit_message** - Generate conventional commit messages
- **api_docs** - Generate API documentation from code
...
# Use a template with ChatBuilder
response = await (
    anthropic
...
# Filter by tag
stratifyai templates --tag code

# View template details
stratifyai templates --verbose

# Use a template in chat
stratifyai chat \
  --template summarize \
  --params "style=bullet_points,max_length=200" \
  --file document.txt
...
Create your own templates in `~/.stratifyai/prompts/`:

```yaml
# ~/.stratifyai/prompts/my_template.yaml
...
See [PROMPT-TEMPLATES.md](PROMPT-TEMPLATES.md) for complete documentation.

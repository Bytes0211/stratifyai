...
A SQLite-backed cache that survives process restarts. Drop-in replacement for `ResponseCache`.
...
class PersistentResponseCache:
    ...
    def get(self, key): ...      # Returns ChatResponse or None
    def set(self, key, response): ...
    def clear(self): ...
    def get_stats(self): ...     # Same format as ResponseCache
...
```python
from stratifyai import PersistentResponseCache

cache = PersistentResponseCache(
    db_path="/tmp/my_cache.db",
    ttl=1800,        # 30 minutes
    max_size=500,
)
...
### Using with the `cache_response` Decorator

The decorator accepts a `cache_backend` parameter to swap in persistent storage:

```python
from stratifyai import cache_response, PersistentResponseCache

persistent = PersistentResponseCache(db_path="./cache.db")

@cache_response(ttl=600, cache_backend=persistent)
async def cached_chat(**kwargs):
    return await client.chat(**kwargs)
```

When `cache_backend` is omitted the decorator falls back to the default in-memory `ResponseCache`.
...
**v0.1.3 (2026‑02‑27)**  
- Added `PersistentResponseCache` with SQLite backend  
- Added `cache_backend` parameter to `cache_response()` decorator  
- `ChatResponse.to_dict()` used for serialization/deserialization  

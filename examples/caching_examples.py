...
def example_3_prompt_caching_with_cache_control():
    ...
    messages = [
        ...
        Message(
            role="user", content="Based on the guide above, explain neural networks."
        ),
    ]
    ...
    messages_2 = [
        Message(
            role="system", content=long_context, cache_control={"type": "ephemeral"}
        ),
        Message(
            role="user",
    ...
    if response2.usage.cache_read_tokens > 0:
        ...
        print(
            f"Cost savings: ${response1.usage.cost_usd - response2.usage.cost_usd:.6f}"
        )
...
def example_5_cost_comparison():
    ...
    for i, query in enumerate(queries, 1):
        ...
        print(
            f"{i}. {query[:40]:<40} | Cost: ${response.usage.cost_usd:.6f} | "
    ...
    estimated_cost_without_cache = total_cost_with_cache * (
        len(queries) / (len(queries) - cache_hits)
    )
    savings = estimated_cost_without_cache - total_cost_with_cache
    print(
        f"Estimated savings: ${savings:.6f} ({(savings / estimated_cost_without_cache) * 100:.1f}%)"
    )

...
class Chatbot:
    ...
    def chat(self, user_input: str) -> str | None:
        ...
        try:
            ...
            response = self.client.chat(
                model=self.current_model, messages=self.conversation
            )
    ...
    def load_conversation(cls, file_path: Path) -> "Chatbot":
        ...
        bot.conversation = [
            Message(role=m["role"], content=m["content"]) for m in data["conversation"]
        ]
        ...
        console.print(
            f"[green]✓ Loaded conversation with {len(bot.conversation)} messages[/green]"
        )
    ...
    def list_models(self):
        ...
        for provider, provider_models in models.items():
            ...
            for model in provider_models:
                current = (
                    " [green](current)[/green]"
                    if model == self.bot.current_model
                    else ""
                )
        ...
        while True:
            try:
                ...
                if budget_status.get("budget_set") and budget_status["over_budget"]:
                    overspent = (
                        budget_status["total_cost"] - budget_status["budget_limit"]
                    )
                    console.print(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Interactive multi-provider chatbot")
    parser.add_argument(
        "--model",
        "-m",
        default="gpt-4o-mini",
    ...
    parser.add_argument("--load", "-l", type=Path, help="Load conversation from file")
    ...
    if args.load:
        ...
        bot = Chatbot(
            model=args.model, system_message=args.system, budget_limit=args.budget
        )

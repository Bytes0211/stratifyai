...
"""
        ...
        try:
            ...
            if self.tracker.is_over_budget():
                ...
                console.print(
                    f"[yellow]⚠ Budget exceeded! Overspent by ${overspent:.2f}[/yellow]"
                )
            ...
            return {"summary": None, "error": str(e), "success": False}
    ...
    def print_summary_table(self, results: list[dict[str, Any]]):
        ...
        for result in results:
            ...
            if result["success"]:
                ...
                table.add_row(file_name, "N/A", "N/A", "N/A", "$0.0000", "❌")
...
def main():
    ...
    parser.add_argument(
        "files", nargs="+", type=Path, help="Document files to summarize"
    )
    parser.add_argument(
        "--output", "-o", type=Path, help="Output JSON file for summaries"
    )
    parser.add_argument(
        "--budget",
        "-b",
    ...
    summarizer = DocumentSummarizer(
        budget_limit=args.budget, use_router=not args.no_router
    )

    # Process documents
    results = summarizer.summarize_batch(
        file_paths, max_length=args.max_length, style=args.style
    )

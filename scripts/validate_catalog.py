...
def validate_schema(catalog: dict[str, Any], schema: dict[str, Any]) -> list[str]:
    ...
    if "version" in catalog:
        ...
        if len(parts) != 3 or not all(p.isdigit() for p in parts):
            errors.append(
                f"Invalid version format: {version} (expected semver like 1.0.0)"
            )
        ...
        for model_id, model_data in models.items():
            ...
            for field in required_fields:
                if field not in model_data:
                    errors.append(
                        f"{provider_name}/{model_id}: Missing required field: {field}"
                    )
            ...
            if "context" in model_data:
                ...
                if not isinstance(context, int) or context <= 0:
                    errors.append(
                        f"{provider_name}/{model_id}: Invalid context window: {context} (must be positive integer)"
                    )
            ...
            if model_data.get("deprecated"):
                if "deprecated_date" not in model_data:
                    errors.append(
                        f"{provider_name}/{model_id}: Deprecated models must have deprecated_date"
                    )
                if "replacement_model" not in model_data:
                    errors.append(
                        f"{provider_name}/{model_id}: Deprecated models must have replacement_model"
                    )
...
def check_routing_fields(providers: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for provider_name, models in providers.items():
        for model_id, model_data in models.items():
            if "quality_score" not in model_data:
                warnings.append(
                    f"{provider_name}/{model_id}: Missing quality_score (router will use default 0.75)"
                )
            if "avg_latency_ms" not in model_data:
                warnings.append(
                    f"{provider_name}/{model_id}: Missing avg_latency_ms (router will use default 2000)"
                )
    return warnings
...
def main():
    ...
    if "providers" in catalog:
        ...
        if not provider_errors:
            ...
            print(
                f"✓ Found {len(catalog['providers'])} providers with {total_models} models"
            )

        # Check routing fields (warnings only)
        routing_warnings = check_routing_fields(catalog["providers"])
        if routing_warnings:
            print(f"\n⚠ {len(routing_warnings)} routing field warnings:")
            for warning in routing_warnings:
                print(f"   - {warning}")
        else:
            print("✓ All models have quality_score and avg_latency_ms")

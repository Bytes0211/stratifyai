    ...
    for provider_name, models in providers.items():
        for model_id, model_data in models.items():
            if "quality_score" not in model_data:
                warnings.append(f"{provider_name}/{model_id}: Missing quality_score (router will use default 0.75)")
            if "avg_latency_ms" not in model_data:
                warnings.append(f"{provider_name}/{model_id}: Missing avg_latency_ms (router will use default 2000)")
    return warnings

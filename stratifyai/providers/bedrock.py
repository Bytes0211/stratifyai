...
class BedrockProvider(BaseProvider):
    ...
    def __init__(
        ...
        if not bearer_token and not (
            ...
            try:
                ...
                self.logger.warning(
                    "No explicit Bedrock credentials found from env/api_key. "
                    "Proceeding with IAM role/credential chain fallback: %s",
                    str(exc),
                )
    ...
    def _normalize_response(self, raw_response: dict, model: str) -> ChatResponse:
        ...
        if model.startswith("anthropic.claude"):
            ...
            finish_reason = raw_response.get("results", [{}])[0].get(

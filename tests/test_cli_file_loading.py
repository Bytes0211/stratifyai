...
class TestLoadFileContentSuccess:
    ...
    def test_load_valid_small_text_file(
        ...
        # Mock MODEL_CATALOG
        with patch(
            "cli.stratifyai_cli.MODEL_CATALOG",
    ...
    def test_load_valid_medium_text_file(
        ...
        warning_messages = [
            call for call in print_calls if "⚠ Large file detected" in call
        ]
...
class TestLoadFileContentNotFound:
    ...
    def test_file_not_found_error(
        ...
        assert any(
            str(nonexistent_file) in call or "nonexistent.txt" in call
...
class TestLoadFileContentSizeLimit:
    ...
    def test_file_exceeds_max_size_limit(
        ...
        warning_messages = [
            call for call in print_calls if "consume significant tokens" in call
        ]
    ...
    def test_large_file_warning_with_user_rejection(
        ...
        cancel_messages = [
            call for call in print_calls if "File load cancelled" in call
        ]
...
class TestInteractiveModeInitialFileLoad:
    ...
    def test_interactive_mode_loads_initial_file_with_flag(
        ...
        loading_messages = [
            call for call in print_calls if "Loading initial context" in call
        ]
        assert len(loading_messages) > 0, "Loading context message not displayed"

        # Verify "File loaded as initial context" message
        loaded_messages = [
            call for call in print_calls if "File loaded as initial context" in call
        ]
        ...
        assert test_content in first_request_messages[0].content, (
            "File content not in initial message"
        )
        assert "[Context from context.txt]" in first_request_messages[0].content, (
    ...
    def test_interactive_mode_continues_without_file_if_load_fails(
        ...
        welcome_messages = [
            call for call in print_calls if "StratifyAI Interactive Mode" in call
        ]

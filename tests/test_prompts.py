...
def test_prompt_template_render_with_defaults():
    ...
    template = PromptTemplate(
        ...
        parameters=[
            ...
            PromptParameter(name="max_length", type="number", default=100, required=False),
        ]
    )
...
""")
        ...
        assert template_after.description.strip() in ["My custom template", "Overridden template"]
...
def test_template_messages_work_with_chat_request():
    ...
    messages = registry.render("chatbot", persona="coding assistant", tone="professional")
...
def test_all_builtin_templates_render_with_defaults():
    ...
    for template in templates:
        ...
        required_params = [p for p in template.parameters if p.required and p.default is None]

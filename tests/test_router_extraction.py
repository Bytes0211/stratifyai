...
class TestRouterExtraction:
    ...
    def test_route_for_schema_extraction(self):
        ...
        assert provider in ["anthropic", "openai", "google", "grok"]
    ...
    def test_route_for_error_extraction(self):
        ...
        assert provider in ["deepseek", "openai", "anthropic", "google", "grok"]
        assert isinstance(model, str)
    ...
    def test_route_for_structure_extraction(self):
        ...
        provider, model = router.route_for_extraction(
            file_type=FileType.PYTHON, extraction_mode="structure"
        )

        # Should select code-capable model
        assert provider in ["deepseek", "anthropic", "openai", "google", "grok"]
    ...
    def test_route_with_cost_constraint(self):
        ...
        metadata = router.get_model_info(provider, model)
        avg_cost_per_1k = (
            metadata.cost_per_1m_input + metadata.cost_per_1m_output
        ) / 1000
    ...
    def test_extraction_routing_different_file_types(self):
        ...
        csv_provider, csv_model = router.route_for_extraction(FileType.CSV, "schema")

        # Log should get error extraction
        log_provider, log_model = router.route_for_extraction(FileType.LOG, "errors")
    ...
    def test_extraction_with_excluded_providers(self):
        ...
        router = Router(
            strategy=RoutingStrategy.QUALITY, excluded_providers=["anthropic"]
        )
    ...
    def test_extraction_with_preferred_providers(self):
        ...
        router = Router(
            strategy=RoutingStrategy.QUALITY, preferred_providers=["google"]
        )
